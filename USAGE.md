...
├── homes/                  # Encrypted home volumes
├── keys/                   # Key storage
├── src/
│   ├── manager/            # C manager (server, auth, containers, crypto)
│   │   ├── main.c          # Entry point + CLI dispatch
│   │   ├── server.c        # TCP server, session lifecycle
│   │   ├── database.c      # SQLCipher DB layer
│   │   ├── crypto.c        # RSA/Ed25519 verification
│   │   ├── lxd.c           # LXD container management
│   │   ├── volume.c        # LUKS encrypted volumes
│   │   ├── cli.c           # CLI commands
│   │   └── config.h        # Constants
│   ├── client/             # Go SSH client
│   └── ai/                 # Python AI behavior analysis
│       ├── config.py       # Environment configuration
│       ├── db_access.py    # Shared DB layer (pysqlcipher3)
│       ├── sanitizer.py    # Prompt injection defense (5 layers)
│       ├── anomaly_detect.py    # AI 1: Statistical anomaly detection
│       ├── command_analysis.py  # AI 2: LLM command analysis
│       └── generate_test_data.py # Test data generator
├── wrapper/                # ESP32 Python tools
